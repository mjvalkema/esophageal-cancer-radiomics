...
# Original code of above source rewritten by L.R. de Ruiter and M.J. Valkema, June 2022
...
RadiomicsFeatureSelection <- function(features, outcomes) {
  ...
  threshold1 <- 0.6 # median over 100 training sets should have at least AUC of 0.6
...
CreateTrainTestSplits <- function(outcomes, nTrainTestSplits, trainTestFraction, seed=3) {
  ...
  set.seed(seed)
  indicesTrainTestSplits <- createDataPartition(y = outcomes, p = trainTestFraction, list = F, times = nTrainTestSplits)
